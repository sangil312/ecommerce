...
- 패키지 경로는 `com.dev.core.ecommerce.api.controller.v1.*.usecase`이다
- 오용을 없애기 위해 다른 패키지 경로에서 UseCase를 명시적으로 사용하지 않는다
- UseCase는 Presentation Layer에 존재한다
- UseCase는 *Service 클래스만을 참조 가능하다
- UseCase는 *Finder, *Reader, *Manager 등 Logic Layer의 클래스를 절대 참조할 수 없다
...
- Implement Layer = Handler, Finder, Manager, Reader 등 행위 기반의 명명을 사용한다
...
### 컨트롤러 패키지 위치 규칙
- v1 API: `com.dev.core.ecommerce.api.controller.v1` 패키지
...
## 인증 및 인가에 대한 전략
- 이 프로젝트 서버는 내부 Network에 존재한다
- 외부 Gateway를 통해 인증 및 인가가 완료 된 상태로 API 요청이 들어온다
